...
public class VerifyHighestDepositPolicyForMRBResWithNightCharges extends TestCore{
	...
	ArrayList<String> policyNumberOfNights = new ArrayList<>();
	...
	policyText, reservation=null, status=null, ratePlan1, paymentIconInFolio, highestNightsOfPolicy, 
	tripTaxes,tripRoomCharges, tripTotal, tripPaid, tripBalance, amountToBePaid;
	...
	public void verifyHighestDepositPolicyForMRBRes(String roomClassName, String roomClassAbb, String maxAdults, 
		...
		String testDescription = "Verifying highest amount of deposit policy applied for MRB reservartion with number of nights charges"
				+ "when there are different deposit policies for each room class<br>"
				+ "<a href='https://innroad.testrail.io/index.php?/cases/view/682491' target='_blank'>"
				+ "Click here to open TestRail: C682491</a><br/>";
		...
		try {
         if (!Utility.insertTestName.containsKey(testName)) {
			...
			policyNumberOfNights = Utility.splitInputData(attrValue);
		...
		try {
			...
			policies.createPolicy(driver, test_steps, policyName1, policyType, attr, 
					policyNumberOfNights.get(0), null, null, source, policySeason, roomClassName1, ratePlan1, policyText1, policyDesc1);			
...
//			navigation.policies(driver, test_steps);
			policies.createPolicy(driver, test_steps, policyName2, policyType, attr, 
					policyNumberOfNights.get(1), null, null, source, policySeason, roomClassName2, ratePlan1, policyText2, policyDesc2);			

			int nightsCal1 = Integer.parseInt(policyNumberOfNights.get(0));
			int nightsCal2 = Integer.parseInt(policyNumberOfNights.get(1));
			if (nightsCal1>nightsCal2) {
				policyText = policyText1;
				policyName = policyName1;
				highestNightsOfPolicy = policyNumberOfNights.get(0);
				policyDesc = policyDesc1;
			}else {
				policyText = policyText2;
				policyName = policyName2;
				highestNightsOfPolicy = policyNumberOfNights.get(1);
				policyDesc = policyDesc2;
			}
			test_steps.add("========== Capturing highest nights of deposit policy details should be applied ==========");
			test_steps.add("Highest value of nights from both two policies is : <b>"+highestNightsOfPolicy+"</b>");
...
//			depositAmount=reservationPage.deposit(driver, test_steps, "", "");		
		...
		try {
			amountToBePaid = reservationPage.calculationOfDeposiAmountToBePaid(baseAmounts, attr, highestNightsOfPolicy, tripTaxes, tripRoomCharges);			
	...
	public Object[][] getData() {
		return Utility.getData("verifyNCDepositPolicyMRB", excel);
	}
