...
public class RateV2BulkUpdateRatesAllInnCenterReservations extends TestCore {
	...
	public void rateV2BulkUpdateRatesAllInnCenterReservations(String bulkUpdateType, String dateFormatType,
			...
			String updatedBlockedCount, String roomBlockCount, String bookClassColor, String taxName)
		...
		String[] ratePlanArray = ratePlansName.split("\\|");
		...
		try {
			...
			for (String str : ratePlanArray) {
				...
				getTestSteps.clear();
				getTestSteps = ratesGrid.selectItemsFromDropDowns(driver, "Rate Plan", str);
				testSteps.addAll(getTestSteps);
		...
		try {
			...
			for (String str : roomClassArray) {
				...
				getTestSteps.clear();
				getTestSteps = ratesGrid.selectRoomClass(driver, str);
				testSteps.addAll(getTestSteps);
		...
		try {
			...
			for (String str : channelArray) {
				...
				getTestSteps.clear();
				getTestSteps = ratesGrid.selectItemsFromDropDowns(driver, "Source", str);
				testSteps.addAll(getTestSteps);
		...
		try {
			...
			// app_logs.info("Navigate TapeChart");
			// getTestSteps.clear();
			// getTestSteps = tapeChart.TapeChartSearch(driver, checkInDate,
			// checkOutDate, adultsCount, childrenCount,
			// ratePlansName, getTestSteps);
			// testSteps.addAll(getTestSteps);
			// app_logs.info("==========SELECT ROOM==========");
			// testSteps.add("==========SELECT ROOM==========");
			// // Get the HasMap of Date/Rate From CheckIn To end CheckOut Date
			// HashMap<String, String> ratesAgainstDateTapeChart = new
			// HashMap<String, String>();
			// ratesAgainstDateTapeChart = tapeChart.getTapChartRates(driver,
			// roomClassAbb, checkInDate, checkOutDate,
			// adultsCount, childrenCount, ratePlansName, dateFormatType,
			// numberofDays);
			...
			// tapeChart.verifyRateUpdate(driver, numberofDays, nightlyRate,
			// updateRatesType, rateCurrencyType,
