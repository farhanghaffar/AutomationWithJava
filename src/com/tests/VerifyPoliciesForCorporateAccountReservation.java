...
public class VerifyPoliciesForCorporateAccountReservation extends TestCore {
	...
	public void verifyPoliciesForCorporateAccountReservation(String rateName, String baseAmount, String addtionalAdult,
			...
			String marketSegment, String corpReferral, String corpFirstName, String corpLastName,
			String accountNumber, String corpPhone, String corpAddress, String corpCity, String groupCountry,
			String corpState, String corpPostalcode, String roomPerNight, String firstName,
			String lastName, String lineItemDescription, String roomCharge,
			String guestFolio, String IsAssign, String isChecked, String checkInPolicType,
			...
			String cancelReason, String capture, String property, String checkINPolicyAmount,
			String cancelationPolicyAmount,String NoShowPolicyAmount) throws InterruptedException, IOException {

		String testName = "VerifyPoliciesForCorporateAccountReservation";
		...
		String accountName = corpFirstName + corpLastName;
		...
		try {
			...
			testSteps.add("Successfully Created New RoomClass " + roomClassName + " Abbe : " + roomClassAbbreviation);
			app_logs.info("Successfully Created New RoomClass" + roomClassName + " Abbe : " + roomClassAbbreviation);
		...
		try {
			...
			policies.deleteAllPolicies(driver, getTestSteps, cancelationPolicyType, cancelationPolicyNameWithoutNum);

			getTestSteps.clear();
			getTestSteps = policies.createPolicy(driver, getTestSteps, cancelationPolicyName, cancelationPolicyType, policyFor,
		...
		try {
			...
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(scriptName, testDescription, testCategory, testSteps);
				Utility.updateReport(e, "Failed to Navigate Accounts", testName, "NavigateAdvGroup", driver);
			} else {
				Assert.assertTrue(false);
			}
		} catch (Error e) {
		...
		try {
			...
			app_logs.info("Enter Account Name : " + accountName);
			account.enterAccountNumber(driver, accountNumber);
			testSteps.add("Change Account Number : " + accountNumber);
			app_logs.info("Change Account Number : " + accountNumber);
			getTestSteps.clear();
			getTestSteps = account.AccountAttributes(driver, test, marketSegment, corpReferral, getTestSteps);
			testSteps.addAll(getTestSteps);
			getTestSteps.clear();
			getTestSteps = account.Mailinginfo(driver, test, firstName, lastName, corpPhone,
					corpPhone, corpAddress, "", "", "", corpCity, corpState, corpPostalcode,
					getTestSteps);
			testSteps.addAll(getTestSteps);
		} catch (Exception e) {
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(scriptName, testDescription, testCategory, testSteps);
				Utility.updateReport(e, "Failed to Create Corporate Account", testName, "CorpoarteAccount", driver);
			} else {
				Assert.assertTrue(false);
			}
		} catch (Error e) {
		...
		try {
			...
			getTestSteps.clear();
			getTestSteps = account.clickOnFolioOptions(driver);
			testSteps.addAll(getTestSteps);
			...
			getTestSteps.clear();
			getTestSteps = account.selectDepositPolicy(driver, depositPolicyName);
			testSteps.addAll(getTestSteps);

			getTestSteps.clear();
			getTestSteps = account.verifyDepositPolicy(driver, depositPolicyName);
			testSteps.addAll(getTestSteps);
			...
			getTestSteps.clear();
		    getTestSteps = account.selectCheckInPolicy(driver, checkInPolicyName);
			testSteps.addAll(getTestSteps);

			getTestSteps.clear();
		    getTestSteps = account.verifyCheckInPolicy(driver, checkInPolicyName);
			testSteps.addAll(getTestSteps);
			
			getTestSteps.clear();
			getTestSteps = account.selectCancellationPolicy(driver, cancelationPolicyName);
			testSteps.addAll(getTestSteps);
			
			getTestSteps.clear();
			getTestSteps = account.selectNoShowPolicy(driver, noShowPolicyName);
			testSteps.addAll(getTestSteps);

			getTestSteps.clear();
	    	getTestSteps = account.verifyNoShowPolicy(driver, noShowPolicyName);
			testSteps.addAll(getTestSteps);
				

			getTestSteps.clear();
			getTestSteps = account.accountSave(driver);
			testSteps.addAll(getTestSteps);
			...
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(scriptName, testDescription, testCategory, testSteps);
				Utility.updateReport(e, "Failed to attach policies with account", testName, "FolioTab", driver);
		...
		try {
			...
			testSteps.add("Clicking on New Reservation");
			app_logs.info("Clicking on New Reservation");
		} catch (Exception e) {
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(scriptName, testDescription, testCategory, testSteps);
				Utility.updateReport(e, "Failed to click New Reservation", testName, "EnterAccountNumber", driver);
			} else {
				Assert.assertTrue(false);
			}
		} catch (Error e) {
			...
			try {
			app_logs.info("==========ENTER RESERVATION DETAILS CREATED FROM <b>CORPORATE ACCOUNT</b>==========");
			testSteps.add("==========ENTER RESERVATION DETAILS CREATED FROM <b>CORPORATE ACCOUNT</b>==========");
			...
			getTestSteps.clear();
			getTestSteps = reservationPage.selectRoomWithPolicyUpdation(driver, roomClassName, IsAssign,
					accountName);
			testSteps.addAll(getTestSteps);
			...
			getTestSteps.clear();
			getTestSteps = reservationPage.enter_GuestName(driver, getTestSteps, saluation, firstName, lastName);
			testSteps.addAll(getTestSteps);
			...
			//roomNumber = reservationPage.get_RoomNumber(driver, testSteps, "");
			...
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(scriptName, testDescription, testCategory, testSteps);
				Utility.updateReport(e, "Failed to create new reservaion", testName, "Reservation", driver);
			} else {
				Assert.assertTrue(false);
			}
		} catch (Error e) {
		...
		try {
			...
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(scriptName, testDescription, testCategory, testSteps);
				Utility.updateReport(e, "Failed to verify no show and depost policy in history", testName, "Incidental", driver);
			} else {
				Assert.assertTrue(false);
			}
		...
		try {
			...
			testSteps.add("Verified no line item is displaying in folio line item after selected " + accountName);
		...
		try {
			app_logs.info("==========VERIFICATION CANCELLATION POLICY==========");
			testSteps.add("==========VERIFICATION CANCELLATION POLICY==========");
			...
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(scriptName, testDescription, testCategory, testSteps);
				Utility.updateReport(e, "Failed to Navigate Accounts", testName, "Accounts", driver);
			} else {
				Assert.assertTrue(false);
			}
		} catch (Error e) {
		...
		try {
			app_logs.info("==========SEARCH EXISTING CORPORATE ACCOUNT==========");
			testSteps.add("==========SEARCH EXISTING CORPORATE ACCOUNT==========");
			
			account.accountSearch(driver, testSteps, accountName);
		...
		try {
			...
			try {
				getTestSteps.clear();
				getTestSteps = reservationPage.checkInDates(driver, 0, 0);
				testSteps.addAll(getTestSteps);

				getTestSteps.clear();
				getTestSteps = reservationPage.checkOutDates(driver, +1, 0);
				testSteps.addAll(getTestSteps);
			} catch (Exception e) {
			...
			reservationNumber = reservationPage.get_ReservationConfirmationNumber(driver, getTestSteps);
			//roomNumber2 = reservationPage.get_RoomNumber(driver, testSteps, "");
		...
		try {
			app_logs.info("==========VERIFICATION NOSHOW POLICY==========");
			testSteps.add("==========VERIFICATION NOSHOW POLICY==========");
		...
		try {
			...
			account.account_Search(driver, testSteps, accountName);
*/
		...
		try {
			...
			reservationNumber = reservationPage.get_ReservationConfirmationNumber(driver, getTestSteps);
			//roomNumber3 = reservationPage.get_RoomNumber(driver, testSteps, "");
		...
		try {
			...
			testSteps.add("===================== VERIFICATION OF NO SHOW POLICY AMOUNT=====================");
			...
			testSteps.add("Found : " + expectedNoShowAmount);
			app_logs.info("Found : " + expectedNoShowAmount);
			expectedAmountToPayforNoShow = String.format("%.2f", expectedNoShowAmount);
			
			testSteps.add("Expected amount : $ " + expectedAmountToPayforNoShow);
	...
	public Object[][] getData() {
		...
		return Utility.getData("PoliciesForCorpAccReservation", excel);
	}
