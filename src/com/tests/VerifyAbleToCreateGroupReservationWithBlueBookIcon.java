...
public class VerifyAbleToCreateGroupReservationWithBlueBookIcon extends TestCore {
	...
	public void verifyAbleToCreateGroupReservationWithBlueBookIcon(String rateName, String baseAmount,
			...
			String blueBookClass) throws InterruptedException, IOException {

		String testName = "VerifyAbleToCreateGroupReservationWithBlueBookIcon";
		test_description = "Desktop : Advanced group : Verify able to create group reservation with Blue Book icon.<br>"
				+ "<a href='https://innroad.testrail.io/index.php?/cases/view/682422' target='_blank'>"
				+ "Click here to open TestRail: C682422</a>";
		...
		try {
			...
			String roomnumber = reservation.getRoomNumber(driver, getTestSteps);
			...
			getTestSteps.clear();
			getTestSteps = reservation.VerifyGuestProfileCheckox(driver, Boolean.parseBoolean("flase"));
			testSteps.addAll(getTestSteps);

			getTestSteps.clear();
			getTestSteps = reservation.enter_GuestName(driver, getTestSteps, saluation, blueResFirstName, lastName);
			testSteps.addAll(getTestSteps);
		...
		try {
			...
			getTestSteps.clear();
			advgroup.clickOnGroupsReservationTab(driver, getTestSteps);
			testSteps.addAll(getTestSteps);
			...
			assertEquals(text, blueResFirstName + " " + lastName, "Failed: Guest name Missmatched");
			...
			testSteps.add("'Reservation Number : " + reservationNumber + "', 'Guest Name : " + blueResFirstName + " "
					+ lastName + "', 'BlockName : " + blockName + "', 'Room Class : " + roomClassName + "'");

			app_logs.info("'Reservation Number : " + reservationNumber + "', 'Guest Name : " + blueResFirstName + " "
					+ lastName + "', 'BlockName : " + blockName + "', 'Room Class : " + roomClassName + "'");
			...
			group.ClickReservationName_VerifyPopup(driver, reservationNumber, getTestSteps);
		...
		try {
			...
			Utility.app_logs.info("Expected revenue: " + getRelaseDate);
			testSteps.add("Expected revenue  : " + getRelaseDate);
			testSteps.add("Found: $" + afterExpectedRevenue);
			assertEquals(afterExpectedRevenue, expectedRevenue, "Failed Expected revenue is mismatching");
		...
		try {
			...
			assertNotEquals(bookClass, blueBookClass, "Failed Room book Icon is still blue");
			testSteps.add("Verified room book icon color changed from blue to other");
	...
	public Object[][] getData() {
		...
		return Utility.getData("CreateResFromBlueIcon", excel);
	}
