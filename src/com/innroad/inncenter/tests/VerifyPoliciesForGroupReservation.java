...
public class VerifyPoliciesForGroupReservation extends TestCore {
	...
	public void verifyPoliciesForGroupReservation(String rateName, String baseAmount, String addtionalAdult,
			...
			String groupPhn, String groupAddress, String groupCity, String groupCountry,
			String groupState, String groupPostalcode, String roomPerNight, String firstName,
			String lastName, String lineItemDescription, String roomCharge,
			String guestFolio, String blueBookClass, String IsAssign, String isChecked, String checkInPolicType,
			...
			String checkINPolicyAmount, String cancelationPolicyAmount,
			String NoShowPolicyAmount) throws InterruptedException, IOException {

		String testName = "VerifyPoliciesForGroupReservation";
		test_description = "Verify the policies from Account/groups/quote<br>"
				+ "<a href='https://innroad.testrail.io/index.php?/cases/view/682460' target='_blank'>"
				+ "Click here to open TestRail: C682460</a>";
		...
		try {
			...
			policies.createPolicy(driver, testSteps, depositPolicyName, policyType, policyFor, policyAmount, null, null,
		...
		try {
			app_logs.info("==========ENTER RESERVATION DETAILS CREATED FROM <b>GROUP</b>==========");
			testSteps.add("==========ENTER RESERVATION DETAILS CREATED FROM <b>GROUP</b>==========");

			getTestSteps.clear();
			getTestSteps = reservationPage.clickNewReservationInGroups(driver);
			testSteps.addAll(getTestSteps);
	...
	try {
		testSteps.add(
				"=====================VERIFICATION OF CHECK IN POLICY AMOUNT=====================");
		getTestSteps.clear();
	...
	try {
		...
		if (Utility.reTry.get(testName) == Utility.count) {
			RetryFailedTestCases.count = Utility.reset_count;
			Utility.AddTest_IntoReport(scriptName, testDescription, testCategory, testSteps);
			Utility.updateReport(e, "Failed to verify deposit and checkin policies in history", testName, "History", driver);
		} else {
			Assert.assertTrue(false);
		}
	...
	try {
		...
		app_logs.info("==========VERIFY DEPOSIT POLICY IN GUEST FOLIO==========");
		testSteps.add("==========VERIFY DEPOSIT POLICY IN GUEST FOLIO==========");
	...
	try {
		...
		try {
			...
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(scriptName, testDescription, testCategory, testSteps);
				Utility.updateReport(e, "Failed to to create a new reservation", testName, "Reservation", driver);
			} else {
				Assert.assertTrue(false);
			}
		} catch (Error e) {
		...
		try {
			testSteps.add("=====================DELEETE ALL POLCIES THAT CREATED=====================");
			navigation.Inventory(driver, testSteps);
			navigation.policies(driver, testSteps);
			policies.deleteAllPolicies(driver, testSteps, policyType, depositPolicyNameWithoutNum);
			policies.deleteAllPolicies(driver, testSteps, checkInPolicType, checkInPolicyNameWithoutNum);
	...
	public Object[][] getData() {
		...
		return Utility.getData("PoliciesForGroupReservation", excel);
	}
