...
public class VerifyNoShowPolicyForMRBResWithFixedAmounts extends TestCore{
	...
	policyName1, policyName2, policyText1, policyDesc1, policyText2, policyDesc2, policyDesc, 
	policyText, reservation=null, status=null, ratePlan1, paymentIconInFolio, highestFixedAmount, 
	tripTaxes,tripRoomCharges, tripTotal, tripPaid, tripBalance, amountToBePaid, cardExpDate;
	...
	public void verifyNoShowPolicyForMRBResWithFixedAmounts(String roomClassName, String roomClassAbb, String maxAdults, 
		...
		String testDescription = "Verify No Show policy calculation for MRB when each room has different Fixed amounts<br>"
				+ "<a href='https://innroad.testrail.io/index.php?/cases/view/682492' target='_blank'>"
				+ "Click here to open TestRail: C682492</a><br/>";
		...
		try {
			...
			roomClass.CreateRoomClass(driver, roomClassName2, roomClassAbb2, null,
					maxAdults, maxPersons, roomQuantity, test, test_steps);       
			roomNumberAssigned.add(Utility.RoomNo);
			roomClass.CloseOpenedRoomClass(driver, roomClassName2, test_steps);
		}catch (Exception e) {
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(testName, testDescription, testCatagory, test_steps);
...
//			navigation.policies(driver, test_steps);
			...
			int fixedAmount1 = Integer.parseInt(policyFixedAmounts.get(0));
			int fixedAmount2 = Integer.parseInt(policyFixedAmounts.get(1));
			if (fixedAmount1>fixedAmount2) {
				policyText = policyText1;
				policyName = policyName1;
				highestFixedAmount = policyFixedAmounts.get(0);
				policyDesc = policyDesc1;
			}else {
				policyText = policyText2;
				policyName = policyName2;
				highestFixedAmount = policyFixedAmounts.get(1);
				policyDesc = policyDesc2;
			}
			test_steps.add("========== Capturing highest nights of deposit policy details should be applied ==========");
			test_steps.add("Highest value of fixed amount from both two policies is : <b>"+highestFixedAmount+"</b>");
			test_steps.add("Highest fixed amount of Deposit policy name is : <b>"+policyName+"</b>");
			test_steps.add("Highest fixed amount of Deposit policy text is : <b>"+policyText+"</b>");
			test_steps.add("highest fixed amount of check-in policy Description is : <b>"+policyDesc+"</b>");
		}catch (Exception e) {
			if (Utility.reTry.get(testName) == Utility.count) {
				RetryFailedTestCases.count = Utility.reset_count;
				Utility.AddTest_IntoReport(testName, testDescription, testCatagory, test_steps);
		...
		try {
			test_steps.add("========== Verifying associated No Show policy details at Policies And Disclaimers tab ==========");
			reservationPage.verifyNoShowPolicy(driver, test_steps, policyName, policyText);
			test_steps.add("========== Making Reservation as No Show =====================");
			reservationPage.makeReservationNoShow(driver, test_steps);
			reservationPage.verifyPolicyDetailsOnPaymentPopup(driver, test_steps, policyType, policyName, policyText);
			reservationPage.enableVoidRoomCharge(driver, test_steps, true);
			reservationPage.clickProceedToNoShowPaymentButton(driver, test_steps);
			test_steps.add("===================== Verifying No Show payment popup =====================");
			String amountToBePaid = reservationPage.calculationOfNoShowAmountToBePaid(baseAmounts, totalNights, attr,
					highestFixedAmount, tripTaxes, tripRoomCharges);
			String cardNumWithXXXX = "XXXX " + cardNumber.substring(cardNumber.length() - 4);
			reservationPage.verifyNoShowPaymentPopUp(driver, test_steps, "No Show Payment", cardExpDate, nameOnCard,
					cardNumWithXXXX, paymentMethod, "Capture", amountToBePaid);
			String paymentMethodExp = paymentMethod + " - " + Utility.getCardNumberHidden(cardNumber) + " (" + nameOnCard
					+ ") (Exp. " + cardExpDate + ")";
			test_steps.add("============ Verifying guest and payment details in Noshow payment success popup =====================");
			reservationPage.verifyNoShowPaymentSuccessPopupVerify(driver, test_steps,
					"No Show Successful", "Processed", paymentMethodExp, "Capture", amountToBePaid);
			...
			test_steps.add("===================== Verifying No show coming as note in guest details section =====================");
			reservationPage.verifyNotesDetails(driver, test_steps, policyType, "No Show", null, "auto guest");
			reservationPage.verifyNoteRoomsDetails(driver, test_steps, policyType, roomClassAbbs, roomNumberAssigned);
			test_steps.add("===================== Verifying No show History details =====================");
			reservationPage.click_History(driver, test_steps);
			String expPaymentDesc = "Made a no show payment of $"+amountToBePaid+" using "+paymentMethod+" -XXXX"+
			cardNumber.substring(cardNumber.length()-4)+" ("+cardExpDate+")";
			reservationPage.verifyPaymentDescriptionAtGuestHistory(driver, test_steps, "PAYMENT", expPaymentDesc);
			...
			reservationPage.verifyPrimaryUserFolioDetails(driver, roomNumberAssigned.get(0), roomClassName1, test_steps);
			reservationPage.verifySecondaryUserFolioDetails(driver, roomNumberAssigned.get(1), roomClassName2, test_steps);
	...
	public Object[][] getData() {
		return Utility.getData("verifyFANoShowPolicyMRB", excel);
	}
